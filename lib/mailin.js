...
Mailin.prototype.start = function(options, callback) {
  ...
  function postWebhook(connection, finalizedMessage) {
    return new Promise(function(resolve) {
      ...
      logger.info(
        connection.id + ' Sending request to webhook ' + configuration.webhook,
      );
      ...
      finalizedMessage.attachments.forEach(function(attachment) {
        attachmentNamesAndContent[attachment.generatedFileName] = (
      ...
      req.end(function(err, resp) {
        ...
        if (err || resp.statusCode !== 200) {
          logger.error(
            connection.id +
              ' Error in posting to webhook ' +
              configuration.webhook,
          );
          if (resp) {
            logger.error(
              connection.id + ' Response status code: ' + resp.statusCode,
            );
          }
          return resolve();
        }
